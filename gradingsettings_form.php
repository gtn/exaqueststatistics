...
class quiz_exaqueststatistics_settings_form extends moodleform {
    ...
    /**
     * quiz_exaqueststatistics_settings_form constructor.
     *
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'options', get_string('options', 'quiz_exaqueststatistics'));
        ...
        foreach (['needsgrading', 'manuallygraded', 'autograded', 'all'] as $type) {
            ...
            $gradeoptions[$type] = get_string('gradeattempts' . $type, 'quiz_exaqueststatistics',
                    $this->counts->$type);
        }
        $mform->addElement('select', 'grade', get_string('attemptstograde', 'quiz_exaqueststatistics'),
                $gradeoptions);

        $mform->addElement('text', 'pagesize', get_string('questionsperpage', 'quiz_exaqueststatistics'),
                ['size' => 3]);
        $mform->addRule('pagesize', null, 'positiveint', null, 'client');
        $mform->setType('pagesize', PARAM_INT);

        $orderoptions = [
            'random' => get_string('random', 'quiz_exaqueststatistics'),
        ...
        $mform->addElement('select', 'order', get_string('orderattemptsby', 'quiz_exaqueststatistics'),
                $orderoptions);
        ...
        $mform->addElement('submit', 'submitbutton', get_string('changeoptions', 'quiz_exaqueststatistics'));
    }
}
